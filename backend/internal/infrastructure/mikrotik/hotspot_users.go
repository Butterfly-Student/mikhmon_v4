...
func (c *Client) GetHotspotUsers(ctx context.Context, profile string) ([]*dto.HotspotUser, error) {
...
func (c *Client) GetHotspotUsersByComment(ctx context.Context, comment string) ([]*dto.HotspotUser, error) {
	args := []string{"/ip/hotspot/user/print"}
	if comment != "" {
...
func (c *Client) GetHotspotUserByID(ctx context.Context, id string) (*dto.HotspotUser, error) {
	reply, err := c.RunContext(ctx, "/ip/hotspot/user/print", "?.id="+id)
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetHotspotUserByName(ctx context.Context, name string) (*dto.HotspotUser, error) {
	reply, err := c.RunContext(ctx, "/ip/hotspot/user/print", "?name="+name)
	if err != nil {
		return nil, err
	}
...
func (c *Client) AddHotspotUser(ctx context.Context, user *dto.HotspotUser) (string, error) {
	args := []string{
		"/ip/hotspot/user/add",
		"=name=" + user.Name,
...
func (c *Client) UpdateHotspotUser(ctx context.Context, id string, user *dto.HotspotUser) error {
	args := []string{
		"/ip/hotspot/user/set",
		"=.id=" + id,
	}
...
func (c *Client) RemoveHotspotUser(ctx context.Context, id string) error {
	_, err := c.RunContext(ctx, "/ip/hotspot/user/remove", "=.id="+id)
	return err
}

// RemoveHotspotUsersByComment removes hotspot users by comment (for voucher deletion).
func (c *Client) RemoveHotspotUsersByComment(ctx context.Context, comment string) error {
	users, err := c.GetHotspotUsersByComment(ctx, comment)
	if err != nil {
		return err
	}

	for _, user := range users {
		if err := c.RemoveHotspotUser(ctx, user.ID); err != nil {
			c.logger.Warn("failed to remove hotspot user",
...
func (c *Client) ResetHotspotUserCounters(ctx context.Context, id string) error {
	_, err := c.RunContext(ctx, "/ip/hotspot/user/reset-counters", "=.id="+id)
	return err
}
...
func (c *Client) GetHotspotUsersCount(ctx context.Context) (int, error) {
	reply, err := c.RunContext(ctx, "/ip/hotspot/user/print", "=count-only=")
	if err != nil {
		return 0, err
	}
