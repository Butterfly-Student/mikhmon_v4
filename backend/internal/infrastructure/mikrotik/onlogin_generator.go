...
func (g *OnLoginGenerator) buildFooter(req *dto.ProfileRequest) string {
	...
	case "rem", "remc":
		...
		return "}}"
	case "ntf", "ntfc":
		// Notice mode - same as remove but sets limit-uptime=1s when expired
		return "}}"
	case "0":
		// No expiration - just lock scripts if enabled
		return ""
	default:
		return "}}"
	}
...
func (g *OnLoginGenerator) Parse(script string) *dto.ProfileRequest {
	...
	// Pattern: :put (",expire_mode,price,validity,selling_price,,lock_user,lock_server,");
