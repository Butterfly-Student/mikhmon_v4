...
// HotspotInactiveMonitorHandler handles WebSocket streaming of inactive hotspot users
type HotspotInactiveMonitorHandler struct {
...
// NewHotspotInactiveMonitorHandler creates a new WebSocket handler for hotspot inactive monitoring
func NewHotspotInactiveMonitorHandler(
	routerRepo repository.RouterRepository,
	mikrotikSvc *mikrotik.Manager,
	internalWSKey string,
	log *zap.Logger,
) *HotspotInactiveMonitorHandler {
	return &HotspotInactiveMonitorHandler{
		upgrader: websocket.Upgrader{
		...
		log:           log.Named("ws-hotspot-inactive-monitor"),
	}
}

// Handle handles WebSocket connections for hotspot inactive streaming
func (h *HotspotInactiveMonitorHandler) Handle(c *gin.Context) {
	...
	h.log.Info("Hotspot inactive monitor WebSocket upgraded", zap.Uint64("routerID", routerID))
	...
	for {
		...
		if err != nil {
			h.log.Info("Hotspot inactive monitor connection closed", zap.Uint64("routerID", routerID), zap.Error(err))
			return
		}
	}
}

func (h *HotspotInactiveMonitorHandler) startMonitor(
	...
	go func() {
		...
		cancelFn, err := routerClient.ListenHotspotInactive(ctx, resultChan)
		if err != nil {
			h.log.Error("Failed to start hotspot inactive monitor", zap.Error(err))
			conn.WriteJSON(gin.H{"type": "error", "message": "failed to start monitor: " + err.Error()})
			return
		}
		defer cancelFn()

		conn.WriteJSON(gin.H{"type": "status", "status": "started", "monitor": "hotspot-inactive"})
		...
		for {
			...
			case result, ok := <-resultChan:
				...
				if err := conn.WriteJSON(result); err != nil {
					h.log.Warn("Failed to write hotspot inactive result", zap.Error(err))
					return
				}
			}
		}
