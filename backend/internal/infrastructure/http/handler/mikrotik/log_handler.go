...
// GetAllLogs handles GET /logs/all?limit=N&topics=X
func (h *LogHandler) GetAllLogs(c *gin.Context) {
	...
	logs, err := h.logUC.GetLogs(c.Request.Context(), uint(routerID), topics, limit)
	if err != nil {
		h.ErrorWithCode(c, http.StatusBadGateway, handler.ErrCodeMikrotikConnection, err.Error())
		return
	}
...
// GetPPPLogs handles GET /logs/ppp?limit=N
func (h *LogHandler) GetPPPLogs(c *gin.Context) {
	routerID, _ := strconv.ParseUint(c.Param("router_id"), 10, 32)
	limit := 100
	if raw := c.Query("limit"); raw != "" {
		if parsed, err := strconv.Atoi(raw); err == nil && parsed > 0 {
	...
	logs, err := h.logUC.GetPPPLogs(c.Request.Context(), uint(routerID), limit)
	if err != nil {
		h.ErrorWithCode(c, http.StatusBadGateway, handler.ErrCodeMikrotikConnection, err.Error())
		return
	}
...
func (h *LogHandler) RegisterRoutes(r *gin.RouterGroup) {
	...
	r.GET("/logs/all", h.GetAllLogs)
	r.GET("/logs/ppp", h.GetPPPLogs)
}
